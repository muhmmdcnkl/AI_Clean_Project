...
public class VacuumView extends EmptyEnvironmentView<VacuumPercept, Action> {
	...
	public void paintComponent(Graphics g) {
		...
		for (int i = 0; i < locations.size(); i++) {
			...
			if (agent != null) {
				...
				if (action == null || !((DynamicAction) action).getAttribute("name").equals("SWEEP"))
					g2.fillArc(x(11 * i + 2), y(2), scale(6), scale(6), 200, 320);
				else if(action == null || !((DynamicAction) action).getAttribute("name").equals("WIPE"))
					g2.fillArc(x(11 * i + 2), y(2), scale(6), scale(6), 200, 320);
				else
	...
	protected boolean isWet(String location) {
		return VacuumEnvironment.LocationState.Wet == getVacuumEnv().getLocationState(location);
	}
